...
(function($) {
	...
	var editFmContainer ='<div id="edit-form-container"> \
        <form class="form-inline" id="saveFm" action="#"> \
            <div class="modal-body">\
              <div class="form-group" id="new-equipment-container">\
                <input id="new-equipment-value" name="new-equipment-value"  class="form-control"  value=""/>\
              </div> \
            </div>\
			...
			this.each(function() {
				...
				if (value !='' && skipTag != true) {
					...
					if (options.callback && tags_callbacks[id] && tags_callbacks[id]['checkValidity']) {
								...
								if (validationError=='') {
									$('#'+id+'_tagsinput').removeClass('invalid');
								} else{
									$('#'+id+'_tagsinput').addClass('invalid');
								}
					...
					var popOverSettings = 
					{
					    ...
					    content: function () {
					        return '<div id=fm_'+value+'>'+ editFmContainer+'</div>';
					    }
					
					}
					
					var position = tagslist.length;
					var insertPopoverSettings = {
						...
						content: function () {
					        return '<div  id=insertFm_'+position+'>'+ editFmContainer+'</div>';
					    }
					}
					
					
					var insertTag =  $('<span  id=before_'+position+'>').addClass('tag').addClass('insert');
					insertTag.html('<i id=before_'+position+' class="fa fa-plus-square-o" aria-hidden="true"></i>');
					insertTag.find('i')
					...
					insertTag.insertBefore('#' + id + '_addTag');
					
					insertTag.bind('click', function(event){
						$('#insertFm_'+position+' #saveFm #cancel').bind('click', function(event){
							insertTag.popover('hide');
							$('.backdrop').remove();
						});
						
						$('#insertFm_'+position+' #saveFm').submit(function(saveevent){
							var newVal = $('#insertFm_'+position+' #new-equipment-value').val().replace(/ /g,"").toUpperCase();
							if (options.callback && tags_callbacks[id] && tags_callbacks[id]['checkValidity']) {
								var f = tags_callbacks[id]['checkValidity'];
								var validationError = f.call(this, newVal);
								if( validationError != '') {
									$('#insertFm_'+position+' #saveFm #new-equipment-container').addClass('has-error'); 
									$('#insertFm_'+position+' #saveFm #new-equipment-error').text(validationError).show();
									return false;
								}
							}
							...
							insertTag.popover('hide');
							$('.backdrop').remove();
							if (options.callback && tags_callbacks[id] && tags_callbacks[id]['onInsertTag']) {
								var f = tags_callbacks[id]['onInsertTag'];
								f.call(this, position, newVal);
							}
					...
					insertPopover.on('shown.bs.popover', function(event) {
						...
						$('#insertFm_'+position+' #new-equipment-value').focus();
					...
					spanTag.bind('click', function(event){
						...
						if (options.callback && tags_callbacks[id] && tags_callbacks[id]['checkValidity']) {
								var f = tags_callbacks[id]['checkValidity'];
								var validationError = f.call(this, value);
								if (!( validationError == '' || (validationError == 'Duplicate entry' && !spanTag.hasClass('not_valid')))) {
									$('.popover-title').addClass('text-danger');
									$('#fm_'+value+' #saveFm #new-equipment-container').addClass('has-error'); 
									$('#fm_'+value+' #saveFm #new-equipment-error').text(validationError).show();
								}
							}

						$('#fm_'+value+' #saveFm #cancel').bind('click', function(event){
							spanTag.popover('hide');
							$('.backdrop').remove();
						});
						
						$('#fm_'+value+' #saveFm').submit(function(saveevent){
							var newVal = $('#fm_'+value+' #new-equipment-value').val().replace(/ /g,"").toUpperCase();
							...
							spanTag.popover('hide');
							$('.backdrop').remove();
							if (options.callback && tags_callbacks[id] && tags_callbacks[id]['onChangeTag']) {
								var f = tags_callbacks[id]['onChangeTag'];
								var spanTags = $('#'+id+'_tagsinput .text.tag');
									
								f.call(this, spanTags.index(spanTag), newVal);
							}
					...
					spanTagPopover.on('shown.bs.popover', function(event){
						...
						$('#fm_'+value+' #new-equipment-value').focus();
						$('body').keydown(function(e){
						    if (e.which==27){
						        spanTag.popover('hide');
						        $('.backdrop').remove();
		...
		this.each(function() {
         ...
         $(this).attr('data-tagsinput-init', true);
			...
			if (settings.interactive) {
				markup = markup + '<input id="'+id+'_tag" value="" style="height:30px" placeholder="'+settings.defaultText+'" />';
